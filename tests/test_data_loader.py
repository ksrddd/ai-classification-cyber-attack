...
def test_clean_column_names_strips_whitespace() -> None:
    """CICIDS columns ship with a leading space; loader strips them."""
...
def test_stratified_subsample_preserves_class_distribution(synthetic_cicids_df) -> None:
    ...
    # Allow some slack for rounding + min-per-class enforcement.
    assert abs(len(sub) - n) <= len(df[LABEL_COLUMN].unique())
